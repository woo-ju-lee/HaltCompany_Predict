...
primary_ratio <- primary %>%
  mutate(
    ...
    법인세차감전순이익_자산총계 = `법인세차감전 순이익` / `자산총계`,
    법인세차감전순이익률 = `법인세차감전 순이익` / `매출액`,
    비영업손익자산총계 = (`법인세차감전 순이익` - `영업이익`) / `자산총계`,
    ROA = `당기순이익(손실)` / `자산총계`,
    ROE = `당기순이익(손실)` / `자본총계`,
    영업이익_자산총계 = `영업이익` / `자산총계`,
    영업이익률 = `영업이익` / `매출액`,
    순이익률 = `당기순이익(손실)` / `매출액`,
    매출액증가율 = (`매출액` - lag(`매출액`)) / abs(lag(`매출액`)),
    ROA변화 = (`당기순이익(손실)` / `자산총계`) - lag((`당기순이익(손실)` / `자산총계`)),
    자본잠식률변화 = ((`자본금` - `자본총계`) / `자본금`) - lag(((`자본금` - `자본총계`) / `자본금`)),
    자본총계증가율 = (`자본총계` - lag(`자본총계`)) / abs(lag(`자본총계`)),
    
    #성장성
    
    log_자산총계 = log(`자산총계`),
    log_매출액 = log(`매출액`),
    순손실_2년 = (`당기순이익(손실)` < 0) == 1 & lag(`당기순이익(손실)` < 0) == 1,
    영업손실_2년 = (`영업이익` < 0) == 1 & lag(`영업이익` < 0) == 1,
